import { AddEventBody, CustomerAttributesBody, CustomerPropertiesBody, ExportAllCustomersBody, ExportEventsBody } from "./interfaces/requestOptions";
...
export class ExponeaAPI {
    ...
    public async addEvent(event: AddEventBody): Promise<void> {
        const response = await this.wrapper.path("/track/v2/projects/{projectToken}/customers/events", this.projectToken).post({
            body: event,
        });
    ...
    public async retrieveCustomerAttributes(attributes: CustomerAttributesBody): Promise<AttributeResult[]> {
        const response = await this.wrapper.path("/data/v2/projects/{projectToken}/customers/attributes", this.projectToken).post({
             body: attributes,
        });
    ...
    public async exporAlltCustomer(exportProperties: ExportAllCustomersBody): Promise<unknown> {
        const response = await this.wrapper.path("/data/v2/projects/{projectToken}/customers/export", this.projectToken).post({
    ...
    public async exportCustomer(customerIds: string[]): Promise<ExportCustomerData> {
        const response = await this.wrapper.path("/data/v2/projects/{projectToken}/customers/export-one", this.projectToken).post({
            body: {
    ...
    public async exportEvent(exportEventProperties: ExportEventsBody): Promise<DataResult[]> {
        const response = await this.wrapper.path("/data/v2/projects/{projectToken}/customers/events", this.projectToken).post({
